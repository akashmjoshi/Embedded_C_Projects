 #include "system.h"

unsigned int  data_received = 0;
uint8_t ReceivedByteCount = 0;
uint8_t ComFail = 0;
uint8_t ComFail_Count = 0;
uint8_t Com_data_recv = 0;
uint8_t Query_sent = 0;

const unsigned char auchCRCHi_exp[] = {
   
} ;

const unsigned char auchCRCLo_exp[] = {
   
} ;



/*---------------------------------------------------------------------------
* Function name       : void DecodeSlave(void)
*     returns         : -
*     arg1            : -
*     arg2            : -
* Created by          : N.Singh
* Date created        : 11/10/10
* Description         : This function used to decode the query received from
* 						the Modbus master in the RS485 MODBUS network 
*---------------------------------------------------------------------------*/
void DecodeSlave(void)
{
	unsigned int  temp_code1,temp_code2,temp_code3;
    unsigned int  check,s; 
    //uint8_t channel = 0; 
    WDTCTL = WDTPW + WDTHOLD;  
    //TACCTL0 &= ~CCIE;
        
		IE2 &= ~UCA0RXIE;
    
    	 Leds |=  COM_LED;
	     for(s=1000;s>0;s--);
	     Leds &= ~COM_LED;     
	  
	    UART_rcv_count = 0;
	    
	    check = modrtuChecksum ( UART_RX_buf, ReceivedByteCount-2 );
		
		temp_code1 = UART_RX_buf[ReceivedByteCount-2] ;
   		temp_code2 = (unsigned int) (temp_code1 << 8) ;
   		temp_code1 = UART_RX_buf[ReceivedByteCount-1] ;
   		temp_code3 = (unsigned int)(temp_code2|temp_code1) ;
	    
		
   		
   		if ( check == (unsigned int) ( temp_code3 ) )
   		{

   				switch (UART_RX_buf[1])
		        {
		        	case MODBUS_READ_INPUT_REGISTERS : 
		        	case MODBUS_READ_HOLDING_REGISTERS : 
		        	                       
                        if( (PV_address == PV_Address_data) && (UART_RX_buf[0] == Slave_ID_data) )
                        {
                        	PV_address  = 9999;
                    	
                        	if(Channel_Enable_data)
                        	ComFail_Count = 0;
                        	
                        	
                        	data_received = (uint16_t)UART_RX_buf[3];
				            data_received = data_received<<8;
				            data_received |= (uint16_t)UART_RX_buf[4];
				            
				            if(data_received >= (65536-9999))
				            {
				            	data_received = 65536 - data_received;
				            	process_value =(float64_t)(data_received); 
				                Process_Negative = 1;
				            }
				            else
				            {
				            	process_value =(float64_t)(data_received); 
				                Process_Negative = 0;
				            }
				            process_value =(float64_t) (process_value/pow(10,dp));
				             
                        }
  
			       default:
			         		 //modbusReturnException ( 0x01 ); 	
			         		 Sending_flag = 0;			         		    			         
	           }
      
   			
   		}
   		
   		
   		timeout_flag = 0;
   		//TACCTL0 |= CCIE;
   		IE2 |= UCA0RXIE;   
}




